...
public class BSTree {
    ...
    private void insert(String s,Node node){
		if(s.compareTo(node.getElement())<0 && node.hasLeft()) insert(s, node.getLeft()); 		 	// (1)
		else if(s.compareTo(node.getElement())<0 && !node.hasLeft()){ 							 	// (2)  
			Node newNode = new Node(s, null, null, node);
			node.setLeft(newNode);
			size++;
		} 
		else if(s.compareTo(node.getElement())>0 && node.hasRight()) insert(s, node.getRight()); 	// (3)
		else if(s.compareTo(node.getElement())>0 && !node.hasRight()){								// (4)
			Node newNode = new Node(s, null, null, node);
    ...
    // (4) if s is greater than the current node and the current node has no left right (!node.hasRight())
    //     then create a new node (call it newNode) 
    //          - containing s, 
    //          - with left null
    //          - with right null
    ...
    private Node find(String s,Node node){
		if(node.getElement()==null) return null;
    ...
    private void bfs(Node node){
		if(node != null){
			...
			Q.add(node);
			while(Q.size() != 0){
				Node printNode = (Node) Q.remove();
				System.out.println(printNode.getElement());
				if(printNode.hasLeft()) Q.add(printNode.getLeft());
				if(printNode.hasRight()) Q.add(printNode.getRight());
			}
		}			
	}
    //
    ...
    private void dfs(Node node){
		if(node != null){
			...
			stack.push(node);
			while(!stack.empty()){
				Node printNode = (Node) stack.pop();
				System.out.println(printNode.getElement());
				if(printNode.hasLeft()) stack.push(printNode.getLeft());
				if(printNode.hasRight()) stack.push(printNode.getRight());
			}
		}
	}
    //
