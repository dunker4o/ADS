...
public class StringList {
    ...
    public boolean equal(StringList l){	/*{return toString().equals(l.toString());}*/
		if(size()!=l.size()) return false;
		else if(isEmpty()) return true;
		else {
			Node cursorA = head;
			Node cursorB = l.getHead();
			long i = size();
			while(i!=0){
				if(!cursorA.getElement().equals(cursorB.getElement())) return false;
    ...
    public StringList reverse(){
		...
		for (int i=0; i<size(); i++){
			newList.addFront(tempHead.getElement());
    ...
    public Node last(){
		Node tempHead = head;
		for (int i=0; i<size()-1; i++) tempHead = tempHead.getNext();
		return tempHead;
	}
    // Implemented
    //
    ...
    public void append(StringList l){last().setNext(l.getHead());}
    // Implemented
    //
    ...
    public int count(String s){
		if(!isEmpty()){
			StringList tempStrList = reverse();
			tempStrList.remove(s);
			return (int)size()-(int)tempStrList.size();
		} else return 0;
	}
    // Implemented
    //
    ...
    public void replace(String s1,String s2){
		Node tempHead = head;
		for(int i=0; i<size(); i++){
			if(tempHead.getElement().equals(s1)) tempHead.setElement(s2);
			tempHead = tempHead.getNext();
		}
	}
    // Implemented
